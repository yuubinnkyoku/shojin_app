...
- DeepWikiの日本語化と構成を追加し、アプリの各機能や設定について詳細な説明を追加 しようとしたが出来てなさそう
- クリップボードベースの設定インポート/エクスポート機能を追加 したっけ 忘れた
- Gitブランチルールを追加

#### 🐛 Bug Fixes

- metaとtest_apiパッケージのバージョンをダウングレード

#### 📝 Documentation

- README.mdにObtainiumバッジを追加し、表示を調整

#### 🔧 Miscellaneous Tasks

- Gemini CLIワークフローを更新
- Android用fastlane設定と関連ファイルを追加・更新し、ビルドプロセスを改善
- Androidメタデータのdefault_languageをja-JPに変更
- パッケージのバージョンを更新 (animations, file_picker, http, crypto, flutter_svg, share_plus, device_info_plus, flutter_local_notifications, flutter_code_editor)
- F-Droid関連のスクリプトとメタデータを整理
